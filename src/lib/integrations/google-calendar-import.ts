...
export async function syncFromGoogleCalendarForStaff(params: {
  ...
  for (const ev of events) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (admin as any).from('gcal_pending_events').upsert(insertPayload, { onConflict: 'staff_id,external_event_id' })
    if (error) result.errors.push(`UPSERT失敗 ${ev.id}: ${error.message}`)
