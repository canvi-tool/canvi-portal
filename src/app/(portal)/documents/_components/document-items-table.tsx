...
export function DocumentItemsTable({
  ...
  const updateItem = (index: number, field: keyof DocumentItem, value: string | number) => {
    ...
    if (field === 'quantity' || field === 'unit_price') {
      ...
      item.amount = (field === 'quantity' ? numValue : item.quantity) * (field === 'unit_price' ? numValue : item.unit_price)
    } else {
            ...
            {items.length === 0 ? (
              ...
              items.map((item, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>
                    {readOnly ? (
                      <span className="text-sm text-right block">{formatCurrency(item.unit_price)}</span>
                    ) : (
                      <Input
                        type="number"
