...
const demoEstimates = [
  { id: 'est-001', number: 'EST-2026-001', project_id: 'pj-001', title: 'Webリニューアル 見積書', amount: 2500000 },
  { id: 'est-002', number: 'EST-2026-002', project_id: 'pj-002', title: 'SNSマーケ運用 見積書', amount: 850000 },
]
...
export default function ContractNewPage() {
  ...
  const [contractNumber] = useState(`PC-2026-${String(Math.floor(Math.random() * 900) + 100).padStart(3, '0')}`)
  ...
  const handleEstimateImport = (estimateId: string) => {
    ...
    const estimate = demoEstimates.find((e) => e.id === estimateId)
    if (estimate) {
      toast.success(`見積書 ${estimate.number} からインポートしました`)
      // In a real app, this would import items from the estimate
      setItems([
        { name: 'Webサイトリニューアル', description: 'デザイン・実装一式', quantity: 1, unit: '式', unit_price: estimate.amount, amount: estimate.amount },
      ])
    }
  }

  const availableEstimates = demoEstimates.filter(
    (e) => e.project_id === selectedProjectId
  )
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="契約書 新規作成"
        description="プロジェクトクライアント向け契約書を作成します"
        actions={
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {selectedProjectId && availableEstimates.length > 0 && (
              <div className="space-y-2 sm:col-span-2">
                <Label>見積書からインポート（任意）</Label>
                <Select value={selectedEstimateId} onValueChange={handleEstimateImport}>
                  <SelectTrigger className="w-full">
                    <SelectValue placeholder="見積書を選択してインポート" />
                  </SelectTrigger>
                  <SelectContent>
                    {availableEstimates.map((est) => (
                      <SelectItem key={est.id} value={est.id}>
                        {est.number} - {est.title}（¥{est.amount.toLocaleString('ja-JP')}）
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
      ...
      <div className="flex items-center justify-end gap-3">
        ...
        <Button variant="outline" onClick={handleSendFreeeSign}>
