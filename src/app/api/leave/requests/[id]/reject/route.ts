...
// POST /api/leave/requests/[id]/reject - 有給申請を却下（管理者のみ）
export async function POST(
  ...
  try {
    ...
    const parsed = leaveApprovalSchema.safeParse({ action: 'reject', ...body })
    ...
    if (leaveRequest.status !== 'pending') {
      return NextResponse.json(
        { error: `現在のステータス(${leaveRequest.status})では却下できません。申請中の有給のみ却下可能です。` },
        { status: 400 }
      )
    }
